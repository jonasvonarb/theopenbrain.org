    ...
    :class="
      store.activeMenu
        ? route.name != 'home'
          ...
          : 'w-screen bg-gray-300 text-black overflow-x-scroll overflow-y-hidden'
  ...
  >
    ...
    <transition name="fade">
      <ul
        ...
        class="flex relative pt-20 pb-52 w-[35vw] flex-row overflow-visible duration-700"
        :class="
      ...
      >
        ...
        >
          <RouterLink
            ...
            :to="
              'the-eye-and-retina' ==
              menu[index].title.replaceAll(' ', '-').toLowerCase()
                ? '/chapter/the-eye-and-retina/'
                : '/chapter/chaptertocome/' +
                  menu[index].title.replaceAll(' ', '-').toLowerCase()
            "
          >
            <h2 class="pb-5">{{ menu[index].title }}</h2>
          </RouterLink>
          <ul class="pl-6" v-for="(part, i) in chapter.parts">
